...
resource "aws_instance" "worker" {
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo chmod 0755 /tmp/install.sh",
      "sudo /tmp/install.sh worker"
    ]
  }
...
resource "aws_instance" "controller" {
  ...
  provisioner "file" {
    #source      = "${var.boundary_bin}/boundary"
    source      = "${path.module}/source/boundary"
    destination = "/tmp/boundary"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo chmod 0755 /tmp/install.sh",
      "sudo /tmp/install.sh controller"
    ]
  }
...
resource "aws_security_group_rule" "allow_http_web_worker" {
  type              = "ingress"
  from_port         = 80
  to_port           = 80
  protocol          = "tcp"
...
resource "aws_security_group_rule" "allow_https_web_worker" {
  type              = "ingress"
  from_port         = 443
  to_port           = 443
  protocol          = "tcp"
...
# Example resource for connecting to through boundary over SSH
resource "aws_instance" "targetgroup1" {
  count                  = var.num_targets
  ami                    = data.aws_ami.ubuntu.id
  instance_type          = "t3.micro"
  subnet_id              = aws_subnet.private.*.id[count.index]
  ...
  tags = {
    Name        = "${var.tag}-target-group1-${random_pet.test.id}"
...
# Example resource for connecting to through boundary over SSH
resource "aws_instance" "targetgroup11" {
  count                  = var.num_targets
  ami                    = data.aws_ami.ubuntu.id
  instance_type          = "t3.micro"
  subnet_id              = aws_subnet.private.*.id[count.index]
  ...
  tags = {
    Name        = "${var.tag}-target-group11-${random_pet.test.id}"
    Environment = "development"
  }
