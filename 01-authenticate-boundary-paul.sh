...
boundary authenticate password -addr=$BOUNDARY_ADDR -auth-method-id=$AUTHMETHOD       -login-name=paul -password=<PASSWORD> -keyring-type=none -format=json | jq -r ".item.attributes.token" > /tmp/boundary_token.txt
