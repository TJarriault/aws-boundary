...
resource "boundary_role" "global_anon_listing" {
  ...
  description = "global_anon_listing"
...
resource "boundary_role" "org_admin" {
  ...
  principal_ids = concat(
    ...
    [for user in boundary_user.group1 : user.id],
  )
}
...
resource "boundary_role" "group1_project_admin" {
  name           = "Group1_admin"
  description    = "Administrator role for Group1"
  scope_id       = boundary_scope.org.id
